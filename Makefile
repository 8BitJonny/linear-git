# File content heavily inspired by https://github.com/ahoy-cli/ahoy
...
default:
	go build -ldflags $(LDFLAGS) -v -o ./gli

install:
	go build -ldflags $(LDFLAGS) -v -o $(GOPATH)/bin/gli
...
cross: build_dir
	$(foreach os,$(OS), \
		$(foreach arch,$(ARCH), \
			GOOS=$(os) GOARCH=$(arch) go build -trimpath -ldflags $(LDFLAGS) -v -o ./builds/gli-bin-$(os)-$(arch); \
		) \
	)

	$(foreach arch,$(ARCH),mv ./builds/gli-bin-windows-$(arch) ./builds/gli-bin-windows-$(arch).exe;)

clean:
	rm -vRf ./builds/gli-bin-*
